...
`abalone` and `dameo` and scored it: 21 matches, 5 justified deviations, 12 residue items. Four are
closed; the rest are named in the pull request.
...
**An action the rules refuse raises, and go catches nothing.** `Replay.replayDrop`, `replayPass` and
`replaySelectSquares` each end in `.valueOr(error => sys.error(…))`, as their counterparts do in all
nine game logics, and nothing in `go/` intercepts that. The `Uci` loaders still hand back
`Validated.invalid` for what they can express, but a refusal arriving from the rules travels out.

This is deliberate. The path decides legality where it used to take the recorded action on trust, and
it decides by stricter rules than some stored games were written under — four passes settle the game
here now, and nothing may follow a settlement. An action that reaches it and is refused therefore
means the stored record holds a game the rules say is impossible, which is a fault in the data rather
than a condition to render around. `sys.error` is how this codebase says that, and go does not get
its own answer to the question.
...
persistence layer that quietly drops it looked like the likely status quo rather than an unlikely
accident.

**Checked, and it carries it.** lila reads the field (`BSONHandlers.scala:575`), writes it on insert
(`:899`) and diffs it on update (`GameDiff.scala:273`), all generically, for go along with everything
else. Games already in the database hold empty arrays, so they resume with a superko history that
begins at the resume point — the same situation as resuming from a FEN, which the rules already
account for, and it heals from the next placement because `hashAfterPlacing` falls back to a
full-board recompute when the history is empty.
...
### Position state, and how a resumed game gets it

This is the other half of the same problem, and unlike `positionHashes` it did not have a happy
answer. lila persists a go game as its stones plus its action strings. It has no column for `komi`,
`ko`, `consecutivePasses` or `deadStonesSelected` — before this branch it did not need one, because
`go.Board.apiPosition` was a `lazy val` that replayed `uciMoves` through the engine and the engine
knew all four. The rules moving onto `variant.Variant` left those values as ordinary `Board` fields
with nobody to fill them.

So `go.Board` takes a second `apply`, with the six parameter names `readGoGame` uses, that works them
out from the move list the way the engine did:

- **`komi`** from the starting position's fen, else the variant's. A handicap game scored by its
  variant's komi turns a draw into a win.
- **`consecutivePasses`** is the trailing run of `pass`; **`deadStonesSelected`** is a recorded
  `ss:`. Both O(1). Without them a game where both players had passed lost its dead-stone offer after
  any cache miss, because `Situation.canSelectSquares` reads `consecutivePasses` where it used to
  read `uciMoves`.
- **`halfMoveClock`** from the initial ply plus the move count — see the entry below on what lila
  fills it with.
- **`ko`** is the only one that cannot be arithmetic, because it is a fact about the position
  *before* the last move. It replays — but only behind a gate: simple ko needs the placed stone to
  stand alone with exactly one liberty, and both of those are readable off the stones lila supplies.
  That shape is rare, so almost every restored board answers `None` for the cost of one chain walk.
  A replay the rules refuse yields `None` rather than throwing.

A run of four passes is counted, not settled. `Variant.settlesByPassing` would settle it on replay
and deliberately does not here: that is the behaviour a resumed game already had, and making the
restore settle would end games that are currently live in the database.

`GoLilaBoundaryTest` is the only exercise this path has — nothing else in the repository builds a
board this way — so it should outlive neither more nor less than the seam it tests.

### `go.Api` is reduced to a data holder

Almost all of it is gone. What survives is the shape lila's stored-game reader constructs with, and
nothing behind it: `Api.Position(initialFen, uciMoves)` and `positionFromStartingFenAndMoves`, which
`readGoGame` names and `Rematcher` and `SgfDump` read `initialFen` off. No engine, no position
logic — `go.Board` works the position out from the same two values, using the rules in
`variant.Variant`. `go/Api.scala` carries the `TODO(lila)` that deletes it: persist `komi`, `ko`,
`consecutivePasses` and `deadStonesSelected`, and neither it nor `Board.uciMoves` is needed.

Everything else in the table below is deleted with no replacement in the seam.
...
| `Api.Position.initialFen` | **kept** | the one member of `Api.Position` anything reads. `go.Board.apiPosition.initialFen` answers it, falling back to the variant's own start when the game named none |
...
- **`go.History.score` still exists, and is vestigial.** Nothing in the go package reads or writes
  it. The area score is derived — `Board.areaScore` — and that is what `strategygames.History.Go`
  reports, so a caller that sets this field and reads `strategygames.History.score` back gets the
  position's own score rather than the value it set. It survives because `readGoGame` names it, and
  it carries the `TODO(lila)` that deletes it. Its *meaning* on the go path has changed — see below.
...
### A go `Board` no longer produces a `FEN`-typed value
...
`go.format.FEN`'s accessors index by full-FEN field positions, so a `FEN`-typed projection of a bare
board would be a lie in the type. Build a `Game` and call `Forsyth.>>` for a `FEN`; take the player
from the `Situation` that has one. `fairysf` and `samurai` keep their own `exportBoardFen` — only
go's is gone.

**`Forsyth.exportBoard(board): String` is unchanged**, and still returns all ten fields. Board-only
state is what every other logic returns there, and the conformance review was right to ask for it —
but the string is a published wire format. lila hands it to `MoveGameEvent`, the `/tv/feed` stream
and the user-games API, and readers index it by field: `stratops`' go variant reads fields 3 and 4
for the scores, lila's own `ui/stratutils` reads field 1 for the turn. Dropping two fields shifts
every one of those by one, at runtime, with nothing red. The site carries the `TODO(lila)` for
narrowing it once those readers take the player and the full move from somewhere else.

The `Board` accessors above went anyway, and `Situation.unary_!` is `copy(player = !player)` as it is
in all eight sibling logics — those were the valuable half of that change and they are independent of
the field count.
...
**`Forsyth.exportBoard` and `boardAndPlayer` keep their field counts** — ten and eleven. That was
not true for part of this branch's life, and it is worth knowing it was considered and reverted; see
the section above.

**`>>(game)` now takes the turn symbol from `game.situation.player`**, where it used to derive it
from `board.history.halfMoveClock`. Inside the library the two cannot disagree — every construction
site was checked — but a caller that builds `strategygames.Situation(GameLogic.Go(), board, player)`
with a mismatched pair now gets its own player exported rather than the board's. The new answer is
the more honest one; the point is that the invariant is no longer enforced anywhere, and the one test
that asserted it went with the member it read.

A 478-line corpus — three initial positions, seven scripted games at every ply, nine handicap games,
the eight upstream 19x19 games, each recorded as `>>(game)`, `>>(situation)`, `>>(!situation)` and a
reload round trip — was dumped before and after and diffed identical.
...
`strategygames.History.halfMoveClock` is a pass-through field, so any lila code keyed on the raw
value for a go game (a clock, a move counter, a stored-game migration) reads a different number for
the same game, with no compile error.

One reader was found and is handled. lila does not store `halfMoveClock` for go — `readGoGame`
derives one with a chess heuristic, plies since the last capture or pawn move, which answers `0` for
every go game because every go action string begins with a lower-case letter. Left alone that made
`Forsyth.fullMovePart` export full move `1` for every stored game. The compatibility `Board.apply`
replaces the value with the one the move list proves, and `GoLilaBoundaryTest` pins it.
...
**`pgn.Parser.sans` is still a stub**, answering `Not implemented iterable moves` for every go action
string. Nothing in `go/` routes through it any more. `Replay.situations`, `Replay.boards` and
`Replay.plyAtFen` used to, which meant none of their bodies had ever executed; they read `Uci`
instead now, as the rest of go does. `Replay.situations` remains reachable only from the wrapper
layer, and `recursiveSituations` — the `San`-based descent it used to call — is unreferenced.

**`Ecopening.fromGame` resolves for go now.** It calls `Replay.boards`, so the stub used to refuse
every input before a board was built. It returns the starting position of the default variant, which
is all the go ECO table holds — three rows, one per board size. lila calls
`chess.opening.Ecopening.fromGame` and never go's, so the change is not visible downstream.
