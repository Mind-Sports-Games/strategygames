...
class GoLongGameTest extends Specification with ValidatedMatchers {
  ...
  // JOANSALA ORACLE: the three capacity-boundary examples pin the parked joansala engine's GoGame
  // array-capacity fix (the scala engine has no such capacity); deleted with it in Phase C.
  "A Go 19x19 game with more than 602 total makeMove calls (capacity boundary)" should {
    "not throw ArrayIndexOutOfBoundsException when making 603+ moves via makeMovesNoLegalCheck" in {
      ...
      val stoneMoves = (0 until 300).map(i => Api.moveToUci(i, variant.Go19x19Joansala)).toList
      ...
      val result = Api.position(variant.Go19x19Joansala).makeMovesNoLegalCheck(allMoves)
  ...
  "A Go 19x19 game with all 361 positions played (P1 from index 0, P2 from last index)" should {
    "still have legal moves available after filling the board" in {
      ...
      val allMoves = p1Indices
        .zip(p2Indices)
        .flatMap { case (a, b) =>
          List(Api.moveToUci(a, variant.Go19x19Joansala), Api.moveToUci(b, variant.Go19x19Joansala))
        } :+ Api.moveToUci(half, variant.Go19x19Joansala) // remaining P1 move at index 180
  ...
  "A Go 19x19 deep-copied position with 602 moves (simulating validDrops afterDrop path)" should {
    "not throw ArrayIndexOutOfBoundsException when making one more move on the deep copy" in {
      ...
      var position = Api.position(variant.Go19x19Joansala)
      (0 until 602).foreach { _ =>
        val drops = position.legalDrops
        position = position.makeMovesNoLegalCheck(List(Api.moveToUci(drops(0), variant.Go19x19Joansala)))
      }
      val deepCopy = position.deepCopy
      val oneMore  = Api.moveToUci(position.legalDrops(0), variant.Go19x19Joansala)
