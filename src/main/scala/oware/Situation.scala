...
import strategygames.oware.format.{ Forsyth, Uci }
...
case class Situation(board: Board, player: Player) {
  ...
  def withVariant(variant: strategygames.oware.variant.Variant) =
    copy(
...
object Situation {
  ...
  def apply(variant: strategygames.oware.variant.Variant): Situation =
