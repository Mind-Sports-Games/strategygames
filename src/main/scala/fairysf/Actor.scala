...
final case class Actor(
  ...
  //stub
  lazy val moves: List[Move] = List()
    //kingSafetyMoveFilter(trustedMoves(board.variant.allowsCastling))

  /** The moves without taking defending the king into account */
  //stub
  def trustedMoves(withCastle: Boolean): List[Move] = List()//{
  ...
  //stub
  lazy val check: Boolean = false//board check color
  ...
  //  } yield {
  //    rookPos :: {
  //      if (kingPos.file == File.E && List(File.A, File.H).contains(rookPos.file))
  //        newKingPos :: Nil
  //      else Nil
  //    }
...
object Actor {
  ...
  //  */
  //stub
  def posAheadOfPawn(pos: Pos, color: Color): Option[Pos] = None//pawnDirOf(color)(pos)
