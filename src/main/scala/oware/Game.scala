...
import strategygames.oware.format.{ pgn, Uci }
...
object Game {
  def apply(variant: strategygames.oware.variant.Variant): Game =
    new Game(Situation(Board init variant, P1))

  def apply(variantOption: Option[strategygames.oware.variant.Variant], fen: Option[FEN]): Game = {
    val variant = variantOption | strategygames.oware.variant.Variant.default
    val g       = apply(variant)
    fen
      .flatMap {
