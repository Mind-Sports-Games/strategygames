...
case class Board(
  ...
  private def withCrazyData(data: PocketData)         = copy(pocketData = Option(data))
  ...
  private def ensureCrazyData = withCrazyData(pocketData | PocketData.init)
...
object Board {
  ...
  private def apply(pieces: Iterable[(Pos, Piece)], castles: Castles, variant: Variant): Board =
