...
class FairyFENTest extends Specification with ValidatedMatchers {
  ...
  "Shogi Initial FEN" should {
    "have P1 as the player" in {
      Shogi.initialFen.player === Some(Player.P1)
    }
    "have P2 as the player when inverting" in {
      Shogi.initialFen.invertPlayer.flatMap(_.player) === Some(Player.P2)
    }
  }

  "Example Xiangqi FEN" should {
    "have P2 as the player" in {
      FEN("9/5kR1C/9/9/9/9/9/5A2B/9/4K4 b - - 0 1").player === Some(Player.P2)
    }
    "have P1 as the player when inverting" in {
      FEN("9/5kR1C/9/9/9/9/9/5A2B/9/4K4 b - - 0 1").invertPlayer.flatMap(_.player) === Some(Player.P1)
    }
    "have P2 as the player when inverting the invert" in {
      FEN("9/5kR1C/9/9/9/9/9/5A2B/9/4K4 b - - 0 1").invertPlayer.flatMap(_.invertPlayer.flatMap(_.player)) === Some(Player.P2)
    }
  }
