...
sealed abstract class BoardType(
  ...
  final val cellList: List[Pos] = Range(height - 1, -1, -1)
    .flatMap(y =>
      Range(0, width)
        .filter(x => isCell(x, y))
        .map(x => new Pos(x, y))
  ...
  final val cellListForBinary: List[Pos] = Range(0, height)
    .flatMap(y =>
      Range(0, width)
        .filter(x => isCell(x, y))
        .map(x => new Pos(x, y))
    ...
    .toList
  final val cellSetForBinary: Set[Pos]   = cellListForBinary.toSet
