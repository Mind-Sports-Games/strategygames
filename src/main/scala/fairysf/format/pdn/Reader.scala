...
object Reader {
  ...
  def movesWithSans(
    ...
    Parser.moves(moveStrs, variant.Variant.default) map { moves =>
  ...
  private def makeReplay(game: Game, sans: Sans, iteratedCapts: Boolean = false): Result = {
  ...
  private def makeGame(tags: Tags) = {
    val g = Game(
      variantOption = Some(variant.Variant.default),
      fen = Some(variant.Variant.default.initialFen)
    )
    g.copy(
