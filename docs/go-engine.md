...
Every loader reads action strings as `Uci`. `pgn.Parser.sans` is a stub that refuses every go action
string, and nothing in `go/` calls it; `Replay.situations`, `Replay.boards` and `Replay.plyAtFen`
went through it before this branch, which is why none of their bodies had ever run.
