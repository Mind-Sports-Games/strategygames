...
class PerftTest extends ChessTest {
  ...
  def perft(game: Game, depth: Int): Int = {
    if (depth > 0) {
      val movesList = getMovesList(game.situation.moves.values.flatten)
      movesList.foldLeft(0)((p: Int, move: Move) =>
  ...
  "calculate standard chess perfts" should {
    ...
    "gentest-1364" in {
      val game = Game(Option(Chess960), Option(FEN("8/4k3/2r1Br2/Pp4Q1/3N4/1p5K/1b6/8 b - -")))
      perft(game, 3) === 18802
    }
    "gentest-2309" in {
      val game = Game(Option(Chess960), Option(FEN("7k/8/1p2p2n/Pp2P2p/7P/Q1p1P3/3NK3/4N2B b - -")))
      perft(game, 3) === 5876 // TODO: double check it this is actually correct long term
    }
    "gentest-2698" in {
    ...
    "gentest-3523" in {
      ...
      perft(game, 3) === 1593 // TODO: double check if this is actually correct long term
    }
    "gentest-4253" in {
    ...
    "gentest-4309" in {
      val game =
        Game(Option(Chess960), Option(FEN("1nb1qbn1/4rk1r/ppNp3p/5ppP/P5P1/1P3P2/2PPP1RN/1R1QKB2 b - -")))
      perft(game, 3) === 20587
    }
    "gentest-5008" in {
    ...
    "gentest-5569" in {
      ...
      perft(game, 3) === 54992 // TODO: double check if this is actually correct long term
    }
    "gentest-6195" in {
      val game =
  ...
  "calculate tricky perfts" should {
    ...
    "position 4 mirrored" in {
      ...
      perft(game, 3) === 14975 // TODO: double check if this is actually correct long term
    }
