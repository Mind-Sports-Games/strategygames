...
object Reader {
  ...
  def movesWithSans(moveStrs: Iterable[String], op: Sans => Sans, tags: Tags): Validated[String, Result] =
    Parser.moves(moveStrs, tags.owareVariant | variant.Variant.default) map { moves =>
    ...
    sans.value.foldLeft[Result](Result.Complete(Replay(game))) {
      case (Result.Complete(replay), san) =>
        san(StratSituation.wrap(replay.state.situation)).fold(
          ...
          move => Result.Complete(replay addMove StratMove.toOware(move))
        )
  ...
  private def makeGame(tags: Tags) = {
    val g = Game(
      variantOption = tags(_.Variant) flatMap strategygames.oware.variant.Variant.byName,
      fen = tags.owareFen
    )
    g.copy(
