...
object Game {
  def apply(variant: strategygames.mancala.variant.Variant): Game =
    new Game(Situation(Board init variant, P1))

  def apply(variantOption: Option[strategygames.mancala.variant.Variant], fen: Option[FEN]): Game = {
    val variant = variantOption | strategygames.mancala.variant.Variant.default
    val g       = apply(variant)
    fen
      .flatMap {
