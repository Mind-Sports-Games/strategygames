...
case class Tags(value: List[Tag]) extends AnyVal {
  ...
  def goVariant: Option[strategygames.go.variant.Variant] =
    apply(_.Variant).map(_.toLowerCase).flatMap {
      strategygames.go.variant.Variant byName _
    }

  def backgammonVariant: Option[strategygames.backgammon.variant.Variant] =
    apply(_.Variant).map(_.toLowerCase).flatMap {
  ...
  def dameoVariant: Option[strategygames.dameo.variant.Variant] =
    apply(_.Variant).map(_.toLowerCase).flatMap {
