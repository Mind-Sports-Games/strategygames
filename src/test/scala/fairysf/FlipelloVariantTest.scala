  ...
  "Octagon Flipello variant" should {
    "offers pass when no moves available" in {
      ...
      playActionStrs(actionStrs, variant = Some(variant.OctagonFlipello)) must beValid.like { g =>
        ...
        g.situation.dropsAsDrops.size must_== 0
        g.situation.moves.size must_== 1
      }

      playActionStrs(actionStrs :+ "j9j9", variant = Some(variant.OctagonFlipello)) must beValid.like { g =>
        g.situation.end must_== false
        g.situation.dropsAsDrops.size must_!= 0
        g.situation.moves.size must_== 0
      }
    }
  }
}
