...
case class Game(
  ...
  def apply(move: Move): Game = apply(move, false)

  def apply(move: Move, finalSquare: Boolean): Game = {
  ...
  def isStandardInit = board.pieces == strategygames.fairysf.variant.Shogi.pieces
...
object Game {
  def apply(variant: strategygames.fairysf.variant.Variant): Game = new Game(
  ...
  def apply(variantOption: Option[strategygames.fairysf.variant.Variant], fen: Option[FEN]): Game = {
    val variant = variantOption | strategygames.fairysf.variant.Shogi
    val g       = apply(variant)
    fen
      .flatMap {
