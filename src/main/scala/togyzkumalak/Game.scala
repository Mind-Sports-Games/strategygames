...
import strategygames.togyzkumalak.format.{ FEN, pgn }
...
object Game {
  ...
  def apply(variantOption: Option[strategygames.togyzkumalak.variant.Variant], fen: Option[FEN]): Game = {
    val variant = variantOption | strategygames.togyzkumalak.variant.Variant.default
    val g       = apply(variant)
    fen
      .flatMap {
